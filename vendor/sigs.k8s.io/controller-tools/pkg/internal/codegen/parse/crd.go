		...
		for _, version := range group.Versions {
			for _, resource := range version.Resources {
				if IsAPIResource(resource.Type) {
					resource.JSONSchemaProps = b.typeToJSONSchemaProps(resource.Type, sets.NewString(), []string{}, true)
...
// typeToJSONSchemaProps returns a JSONSchemaProps that describe the JSONSchema validations for the given type.
func (b *APIs) typeToJSONSchemaProps(t *types.Type, found sets.String, comments []string, isRoot bool) v1beta1.JSONSchemaProps {
	// Special cases
	...
	switch t.Kind {
	case types.Builtin:
		v = b.parsePrimitiveValidation(t, found, comments)
	case types.Struct:
		v = b.parseObjectValidation(t, found, comments, isRoot)
	case types.Map:
		v = b.parseMapValidation(t, found, comments)
	case types.Slice:
		v = b.parseArrayValidation(t, found, comments)
	case types.Array:
		v = b.parseArrayValidation(t, found, comments)
	case types.Pointer:
		v = b.typeToJSONSchemaProps(t.Elem, found, comments, false)
	case types.Alias:
		v = b.typeToJSONSchemaProps(t.Underlying, found, comments, false)
	default:
...
// parsePrimitiveValidation returns a JSONSchemaProps object that describes the
// validations for the given primitive type.
func (b *APIs) parsePrimitiveValidation(t *types.Type, found sets.String, comments []string) v1beta1.JSONSchemaProps {
...
// parseMapValidation returns a JSONSchemaProps object that describes the
// validations for the given map type.
func (b *APIs) parseMapValidation(t *types.Type, found sets.String, comments []string) v1beta1.JSONSchemaProps {
	additionalProps := b.typeToJSONSchemaProps(t.Elem, found, comments, false)
...
// parseArrayValidation returns a JSONSchemaProps object that describes the validations for the given array type.
func (b *APIs) parseArrayValidation(t *types.Type, found sets.String, comments []string) v1beta1.JSONSchemaProps {
	items := b.typeToJSONSchemaProps(t.Elem, found, comments, false)
	items.Description = ""
	props := v1beta1.JSONSchemaProps{
		Type:        "array",
...
// parseObjectValidation returns a JSONSchemaProps object that describes the
// validations for the given object type.
func (b *APIs) parseObjectValidation(t *types.Type, found sets.String, comments []string, isRoot bool) v1beta1.JSONSchemaProps {
	props := v1beta1.JSONSchemaProps{
		Type:        "object",
		Description: parseDescription(comments),
	}
	...
	if !strings.HasPrefix(t.Name.String(), "k8s.io/api") {
		m, required := b.getMembers(t, found)
		props.Properties = m
...
// serializations.
func (b *APIs) getMembers(t *types.Type, found sets.String) (map[string]v1beta1.JSONSchemaProps, []string) {
	...
	for _, member := range t.Members {
		...
		if strat == "inline" {
			m, re := b.getMembers(member.Type, found)
			for n, v := range m {
				members[n] = v
			}
			required = append(required, re...)
		} else {
			m := b.typeToJSONSchemaProps(member.Type, found, member.CommentLines, false)
			members[name] = m
